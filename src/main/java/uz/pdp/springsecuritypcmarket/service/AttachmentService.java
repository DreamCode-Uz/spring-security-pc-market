...
public class AttachmentService {
    ...
    public ResponseEntity<?> saveFile(MultipartFile file) {
        if (file.isEmpty()) throw new NotFoundException("File Empty");
    ...
    public ResponseEntity<?> downloadFile(Integer id, HttpServletResponse response) {
        Optional<AttachmentContent> optionalContent = contentRepository.findByAttachment_Id(id);
        if (!optionalContent.isPresent()) throw new NotFoundException("Attachment not found");
        Attachment attachment = repository.getReferenceById(id);
